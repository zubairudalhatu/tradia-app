...
export default async function WalletReceiptPage({ params }: WalletReceiptPageProps) {
  ...
  if (!user) redirect(`/login?next=/account/wallet/${id}/receipt`);
      ...
      <section className="receipt-print-card rounded-tradia border border-slate-200 bg-white p-8 shadow-sm">
        <div className="flex flex-wrap items-start justify-between gap-6 border-b border-slate-200 pb-6">
          ...
          <div className="text-left sm:text-right">
            <p className="text-sm font-extrabold uppercase text-ember">Wallet Receipt</p>
        ...
        <p className="mt-8 border-t border-slate-200 pt-5 text-xs leading-5 text-slate-500">
          This receipt confirms wallet activity recorded by Tradia. Keep the wallet reference for support and reconciliation.
        </p>
      </section>
    </main>
  );
